    ...
    金(催)>水(運)>木(合)>火(能)>土(構)
        ...
        輸入偵測到的現實世界，由輸入現實和發送代碼來關聯，由"辭典關聯性"分發細項。動態網讀取 "背景節點" 將偵測後的輸入 微觀。#TODO:**另外寫篩選腳本(只保留真實)，現在不寫。
            排序即思考(動態網 以更微觀節點模擬(動量更少) 觀察節點動態 觀察儲存成關係節點) (節點設定吸引與排斥，自動分類)，吸收節點進自身並占據一些多維範圍且互動；排斥非自己.關係."預判"節點或親密的節點(好中壞)(食物、人際關係、免疫系統)：
                邏輯思考(取代代碼):動態網接收到動時先查靜態網，之後執行(含節點深度)，向外作用(動量*多維空間上的意義)
        ...
        動與動拉開的間距形成低壓。  動競爭地流向周圍的低壓， max(周圍的動量/低壓範圍)。
        第二步:
        python理解拓樸關係，自己.關係. ，好的 低壓通道吸收 拓樸；壞的 高壓外殼排斥；中性 一樣的傳遞能量。
        親密度高時超共生，產生***
        第三步:
        創建背景節點(穩定後矯正節點名，(深度最終節點傳回)輸出驗證):字(意義、相似詞、反義詞)、數學(大小正負關係、小數、加減乘除)、時間、、、
            數學:加是中性、減是吸收、乘是查表後中性、除是查表(1/1~1/9)移除分母後相乘分子(移除小數點一位時並乘上10)、排斥是刪掉數字
