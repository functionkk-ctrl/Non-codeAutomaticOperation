...
ask[:, 0] = (id[:, 0] == id[:, 1]).astype(int) & (
    id[:, 1] == id[:, 2]).astype(int)  # 絕對錯誤的問題，不寫正確，因為絕對正確比較難寫，故最後統一翻轉
ask[:, 1] = (id[:, 0] == id[:, 1]).astype(int) & (
    id[:, 1] == id[:, 2]).astype(int)  # 兩次回答相同時此回答為否，且遇到g
# 以下幾行實際只有一次提問
# 前兩次回答相同時，剩下的不隨機且回答為是，翻轉後是為g
ask[:, 2] = (ask[:, 0] == ask[:, 1]).astype(int) & (id[:, 0] != 2).astype(int)

# 前兩次回答不同時，設定題目預期回答為否，兩次回答相同的回答為否，同時其中之一是g，***此次提問中確認
...
print(f"正統推測成功率:{sorce}%，耗時{(time.time()-st):.3f}秒")  # 67.5%
...
ask[:, 0] = (id[:, 0] == id[:, 1]).astype(
    int) & (id[:, 1] == id[:, 2]).astype(int)
ask[:, 1] = (id[:, 0] == id[:, 1]).astype(
    int) & (id[:, 1] == id[:, 2]).astype(int)
# 完美的二維代數分支鎖：，player
ask[:, 2] = (ask[:, 0] == ask[:, 1]).astype(int) & (id[:, 0] != 2).astype(int)
...
# 實測結果保證 100%
print(f"邪門歪道成分:{sorce}%，直接抄答案，耗時{(time.time()-st):.3f}秒")
