...
class Noēsis:
    ...
    def catalog(self):
        ...
        stm.add("交流").add("版本").add(f"s{time.time():.0f}").add(["時間穿越","時間凍結","時間生死","時間橋接"]) # TODO:時間橋接只是好聽，其實沒甚麼，只是不照順序直接和其他版本座新的順序
    ...
    def 十六核(self, 低階, 中階, 高階):
        ...
        def 交流(self): 
            ...
            remove = ["殺掉","埋葬","刪掉","移除","刪除","停用","關閉","禁用","卸載","清除","終止","撤銷","封存","丟掉","消失","扔掉","抹掉","滅掉","廢掉"]
            found=path_all(TEMPLATE_DIRS["communication"],remove)
            if next(found):
                for _,d,f in found:
                    self.stm.交流.版本.f.remove()
        ...
        def 觀察(self):
            ...
            低階 = {
                ...
                "減少依賴": "圖片可獨立 對應人體操作，不依賴其他圖片或完整序列",  # OK， read_json_content(file_path,file_name="肌肉記憶",key=time_file)
...
a = np.diff(speeds)
                    ...
                    for t in read_json_content(TEMPLATE_DIRS["live_capture"],"肌肉記憶","time_file"): # time,png
                        if 降頻[1].name == t[1]: 
                            ...
                            for r,_,目標 in path_all(TEMPLATE_DIRS["user"],"_目標完成".png):
                                ...
                                進度變化2_all=np.mean(np.diff(進度狀態2[:, 0])) # 只計算x[0]為進度變化
                                純淨狀態 = np.array([x for x in 進度狀態2 if list(x) not in zxc])
                                進度變化2純淨 = np.mean(np.diff(純淨狀態[:, 0])) # 只計算x[0]為進度變化
                                ...
                                for r,長遠目標f in found_long:
                                    ...
                                    for 長遠f in 長遠目標f:
                                        ...
                                        for r,d,f in found_af:
                                            if any(["進階行為","前置行為"] in d): # 且符合是基礎行為或前置行為。同層資料夾名稱 進階行為或前置行為
                                                工具型肌肉記憶=(path_all(r/d) for dd in d if ["進階行為","前置行為"] in dd)
                                                潛力=sum(1 
                                                    for ff in f 
                                                    for a in 工具型肌肉記憶  
                                                    if 全能ORB(ff,a,similar=0.9) # 找到在下面
                                                    ) 
                                        ...
                                        if 全能ORB(理想目標f,長遠f,similar=0.8): # 更有效的 降頻動作 
                                            需求.append({
                                                ...
                                                "生存": "斷掉聯絡和離開現場以迴避，打好基礎，流動資產。\n不反饋，隱匿長遠目標。\n洗掉導致生存False的行為。", # 收割價值種子，埋進安穩的土裡。 TODO:等上市後跟新 用戶填入詳細資訊 替換細緻的對話內容
                                            })
                                        else: # 覆蓋掉 降頻動作
                                            需求.append({
                                                "path": r,
            ...
            def 危機處理核():
                # 全給 策略調整 處理?YES
                    # TODO:***停止危險動作 # 危險上升時的動作分支 
                    # TODO:*增加安全的動作，來保護、消除危險源頭
                    # TODO:*了解危機影響的範圍、哪些人、哪些資源受影響，受損情況。
                    # TODO:*可以幫助的支援對象的職業
                pass
