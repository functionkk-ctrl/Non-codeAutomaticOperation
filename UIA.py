...
if __name__ == "__main__":
    ...
    def T(Y, M, D, H):
        ...
        gy, gd, zh = (Y-4)%10, (j+9)%10 / (j+1)%12, ((H+1)//2)%12
        return G[gy]+Z[(Y-4)%12], G[(gy*2+M+2)%10]+Z[(M+1)%12], G[gd]+Z[j%12], G[(gd*2+zh)%10]+Z[zh]
    ...
    engine.rootContext().setContextProperty("Backend", backend) # 建議暫時兩個都給，避免 QML 報錯
